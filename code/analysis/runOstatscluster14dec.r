...
mam_capture_sitemerge <- mam_capture_sitemerge %>% filter(Pineda_Main_food != 'Insectivore' & !(taxonID %in% nottospecies) &!(siteID %in% c('DSNY','DELA','HEAL'))) # Gets rid of ~700.
...
if (task == 1)
{Ostats_bysite2015local <- Ostats(traits = traits_mam15, plots = factor(mam_capture_sitemerge$siteID[i2015]), sp = factor(mam_capture_sitemerge$taxonID[i2015]), reg_pool_traits = siteregpoollist_mam15_local, reg_pool_sp = siteregpoolsp_mam15_local, nperm = N_PERM, stat = STAT)
save(Ostats_bysite2015local, file = '~/data/Ostats14Dec/Ostats_bysite2015_wmedlocal.r')}

if (task == 2)
{Ostats_bysite2015iucn <- Ostats(traits = traits_mam15, plots = factor(mam_capture_sitemerge$siteID[i2015]), sp = factor(mam_capture_sitemerge$taxonID[i2015]), reg_pool_traits = siteregpoollist_mam15_iucn, reg_pool_sp = siteregpoolsp_mam15_iucn, nperm = N_PERM, stat = STAT)
save(Ostats_bysite2015iucn, file = '~/data/Ostats14Dec/Ostats_bysite2015_wmediucn.r')}

if (task == 3)
{Ostats_bysite2015conti <- Ostats(traits = traits_mam15, plots = factor(mam_capture_sitemerge$siteID[i2015]), sp = factor(mam_capture_sitemerge$taxonID[i2015]), reg_pool_traits = continentpool15, reg_pool_sp = continentsp15, nperm = N_PERM, stat = STAT)
save(Ostats_bysite2015conti, file = '~/data/Ostats14Dec/Ostats_bysite2015_wmedconti.r')}
