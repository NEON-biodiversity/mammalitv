# Overlap stats and null models. Don't include any T-stats stuff, to make it faster.
# Last modified: 09 Jan 2017

# Modified 09 Jan 2017: Added null model that randomizes the weights
# Modified 20 Dec 2016: Added harmonic mean
# Modified 25 Oct 2016: Added no-ITV case
# Modified 13 Oct 2016: Added weighted median as well.
# Modified 12 Oct 2016: Added option for weighted mean instead of median.
# Modified 06 Oct 2016: Corrected bug in indexing (see github)
# Modified 05 Oct 2016: Added regional O-stats
...
Ostats <- function(traits, plots, sp, reg_pool_traits, reg_pool_sp, nperm = 99, nullqs = c(0.025, 0.975), stat = 'median', shuffle_weights = FALSE, swap_means = FALSE) {
	...
	# pb <- txtProgressBar(min = 0, max = length(reg_pool_traits), style = 3)
	
	# for (r in 1:length(reg_pool_traits)) {
		# for (t in 1:ncol(traits)) {
			# regional_overlap_norm_rt <- try(community_overlap(traits = reg_pool_traits[[r]][,t], sp = reg_pool_sp[[r]], norm=TRUE), TRUE)
			# regional_overlaps_norm[r, t] <- if (inherits(regional_overlap_norm_rt, 'try-error')) NA else regional_overlap_norm_rt
			# regional_overlap_unnorm_rt <- try(community_overlap(traits = reg_pool_traits[[r]][,t], sp = reg_pool_sp[[r]], norm=FALSE), TRUE)
			# regional_overlaps_unnorm[r, t] <- if (inherits(regional_overlap_unnorm_rt, 'try-error')) NA else regional_overlap_unnorm_rt
		# }
