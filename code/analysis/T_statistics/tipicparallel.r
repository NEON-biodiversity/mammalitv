...
plotregpoollist_mam14_siteonly <- lapply(plots14, function(x) {
  ...
  mam_adults[mam_adults$taxonID %in% spp, c('hindfootLength', 'earLength', 'tailLength', 'totalLength', 'weight')]
})
...
traits_i <- list(traits_mam14, traits_mam15, traits_mam14, traits_mam15)[[task]]
i_i <- list(i2014, i2015, i2014, i2015)[[task]]
pool_i <- list(plotregpoollist_mam14_iucn, plotregpoollist_mam15_iucn, plotregpoollist_mam14_siteonly, plotregpoollist_mam15_siteonly)[[task]]
name_i <- c("tstats_mam_plots14_iucnpool", "tstats_mam_plots15_iucnpool", "tstats_mam_plots14_sitepool", "tstats_mam_plots15_sitepool")[task]

tstats_i <- Tstats(traits = traits_i, ind.plot = factor(mam_adults$plotID[i_i]), sp = factor(mam_adults$taxonID[i_i]), reg.pool = pool_i, nperm = 999)
