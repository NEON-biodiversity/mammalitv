...
transformed parameters {
	real mu[N];
	for (i in 1:N) {
		mu[i] = alpha[siteID[i]] + beta[1] * bio1[i] + beta[2] * bio4[i] + beta[3] * cv_bio1[i] + beta[4] * bio12[i] + beta[5] * bio15[i] + beta[6] * cv_bio12[i] + beta[7] * chao1[i] + beta[8] * mpd_z[i] + beta[9] * mntd_z[i] + beta[10] * ruggedness[i];
	}
}
