...
if (task == 1)
{Ostats_bysite2015local <- Ostats(traits = traits_mam15, plots = factor(mam_capture_sitemerge$siteID), sp = factor(mam_capture_sitemerge$taxonID), reg_pool_traits = siteregpoollist_mam15_local, reg_pool_sp = siteregpoolsp_mam15_local, nperm = N_PERM, stat = STAT, shuffle_weights = SW, swap_means = SM)
save(Ostats_bysite2015local, file = '~/data/Ostats09Jan/Ostats_bysite2015_harmoniclocal_swapmeans.r')}

if (task == 2)
{Ostats_bysite2015iucn <- Ostats(traits = traits_mam15, plots = factor(mam_capture_sitemerge$siteID), sp = factor(mam_capture_sitemerge$taxonID), reg_pool_traits = siteregpoollist_mam15_iucn, reg_pool_sp = siteregpoolsp_mam15_iucn, nperm = N_PERM, stat = STAT, shuffle_weights = SW, , swap_means = SM)
save(Ostats_bysite2015iucn, file = '~/data/Ostats09Jan/Ostats_bysite2015_harmoniciucn_swapmeans.r')}

if (task == 3)
{Ostats_bysite2015conti <- Ostats(traits = traits_mam15, plots = factor(mam_capture_sitemerge$siteID), sp = factor(mam_capture_sitemerge$taxonID), reg_pool_traits = continentpool15, reg_pool_sp = continentsp15, nperm = N_PERM, stat = STAT, shuffle_weights = SW, , swap_means = SM)
save(Ostats_bysite2015conti, file = '~/data/Ostats09Jan/Ostats_bysite2015_harmonicconti_swapmeans.r')}
